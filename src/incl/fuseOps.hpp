...
public:
	 ...
	 */
	~FusePriv() {
		for (std::unordered_map<int, char *>::iterator itr = fd_to_path_.begin();
			 itr != fd_to_path_.end();
			 ++itr)
			free(itr->second);
	}
...
namespace fuse_ops {
	...
	int
	write_buf(const char *path, struct fuse_bufvec *buf, off_t offset, struct fuse_file_info *fi);

	int flock(const char *path, struct fuse_file_info *fi, int op);

	int
	fallocate(const char *path, int mode, off_t offset, off_t length, struct fuse_file_info *fi);
