...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt_long(argc, argv, "c:ho:qvV", long_options, &option_ind)) != -1) {
		switch (opt) {
			...
			case 'o':
				...
				break;
			case 'v':
				config_overrides.log_level_override =
					ConfigOverride<Logger::log_level_t>(Logger::log_level_t::DEBUG);
				break;
			case 'q':
				config_overrides.log_level_override =
					ConfigOverride<Logger::log_level_t>(Logger::log_level_t::NONE);
				break;
			case 'V':
				print_version = true;
				break;
	...
	if (print_version) {
		...
		if (!config_overrides.log_level_override.overridden()
			|| config_overrides.log_level_override.value() >= 1) {
			Logging::log.message(
				u8"   ┓\n"
				u8"└─ ┃ ├─\n"
				u8"└─ ┣ ├─\n"
